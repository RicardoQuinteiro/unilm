...
class LayoutLMv2ForTokenMultiLabelClassification(LayoutLMv2PreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
        self.num_labels = config.num_labels
        self.layoutlmv2 = LayoutLMv2Model(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.multi_label_classifier = nn.Linear(config.hidden_size, config.num_labels)
    ...
    def forward(
        ...
        sequence_output, image_output = outputs[0][:, :seq_length], outputs[0][:, seq_length:]
        sequence_output = self.dropout(sequence_output)
        logits = self.multi_label_classifier(sequence_output)
